...
var isRoute = function isRoute(path) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var menuSlugs = menuData ? JSON.parse(menuData.dataset.slugs.replace(/&quot;/g, '"')) : [];
  window.menuSlugs = menuSlugs;
  var restoreTopMenu = function restoreTopMenu() {
    var active = localStorage.getItem('menuActivo') || menuSlugs[0] || 'adm';
    document.querySelectorAll('.menu-superior a').forEach(function (link) {
      link.classList.toggle('active', link.dataset.target === active);
    });
    menuSlugs.forEach(function (id) {
  ...
  setTimeout(function () {
    return document.body.classList.remove('no-animate');
  }, 100);
  ...
  var rutaGuardada = localStorage.getItem('sidebarActivo');
  var rutaActual = rutaGuardada;
  if (!rutaActual) {
    var linkInicio = Array.from(sidebarLinks).find(function (link) {
      return link.href === location.origin + '/' || link.href === location.origin + '/index';
    });
    if (linkInicio) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function bindEventsToRow(row) {
    ...
    if (amountDevInput) {
      amountDevInput.addEventListener('input', function () {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.desactivarMenu = function (switchElement) {
    var url = switchElement.getAttribute('data-url');
    var isChecked = switchElement.checked;
    var accion = isChecked ? 'activarlo' : 'desactivarlo';
    Swal.fire({
      title: "\xBFEst\xE1s seguro que deseas ".concat(accion, "?"),
      text: 'Puedes cambiar el estado nuevamente después.',
      icon: 'warning',
      showCancelButton: true,
      confirmButtonText: "S\xED, ".concat(accion),
      cancelButtonText: 'Cancelar'
    }).then(function (result) {
      if (result.isConfirmed) {
        window.location.href = url;
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var passConfirmInput = document.getElementById('pass_r');
  var passInputE = document.getElementById('pass_e');
  var passConfirmInputE = document.getElementById('pass_er');
